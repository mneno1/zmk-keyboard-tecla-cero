...
#define SLEEP1_TIMEOUT_MS 60000   // 60 seconds to sleep1 from active
#define SLEEP2_TIMEOUT_MS 180000  // 3 minutes to sleep2 from sleep1
#define SLEEP3_TIMEOUT_MS 600000  // 10 minutes to sleep3 from sleep2
...
static void reset_idle_timer(void) {
    ...
    if (current_mode != POWER_MODE_ACTIVE && split_conn) {
        // Return to active mode immediately (inline, not via work queue to avoid race)
        struct bt_le_conn_param param = {
            .interval_min = ACTIVE_CONN_INTERVAL,
            .interval_max = ACTIVE_CONN_INTERVAL,
            .latency = CONN_LATENCY,
        ...
        if (err == 0) {
            current_mode = POWER_MODE_ACTIVE;
            LOG_INF("Returned to active mode on activity");
        } else {
            LOG_WRN("Failed to return to active mode: %d", err);
        }
    }
